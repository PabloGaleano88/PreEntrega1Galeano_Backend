...
class ProductManager {
    ...
    async addProduct({title, description, price, thumbnail, code, status, category, stock}) {
        ...
        if (title && description && price && thumbnail && code && status && category && stock) {
            try {
                if (!fs.existsSync(this.path)) {
                    ...
                    if (!contenido.find((product) => product.code === code)) {
                        ...
                        return ({status:201,payload:"El producto fue agregado satisfactoriamente"})
                    }
                    else
        ...
        try {
            if (!fs.existsSync(this.path)) {
                ...
                if (producto) {
                    ...
                    return({status:200,payload:`El producto con el id: ${id} se eliminó`})
                }
                else
